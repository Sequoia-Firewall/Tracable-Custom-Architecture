...
def run_train(settings: Settings, logger) -> None:
    ...
    cfg.add_row("Parallel training",f"enabled (max_workers={t.get('max_workers') or 'auto'})"
                                     if t.get("parallel_enabled", False) else "disabled")
    ...
    viz_enabled = out.get("visualization_enabled", False)
    system = SystemHandler.from_settings(settings, logger)
    system.initializeAllSegments(Loud=True, visualization_enabled=viz_enabled)
    ...
    if t.get("parallel_enabled", False):
        # Experimental: train segments concurrently across OS processes
        # instead of one after another. 1.1.4 validation (4 paired trials on
        # varied data samples): ~3.2x average wall-clock speedup; the R2 gap
        # vs sequential was not statistically distinguishable from noise at
        # that sample size (parallel even won outright on 1 of 4 trials).
        # Neither parallel method supports per-epoch visualization graphs or
        # (for the partitioned case) feature pruning — each worker is an
        # isolated process, so warn rather than silently drop the setting.
        if viz_enabled:
            console.print("[yellow]Note:[/yellow] output.visualization_enabled has no effect "
                          "under parallel training (per-epoch graphs require single-process "
                          "execution) — ignoring.")
        max_workers = t.get("max_workers")
        if m["training_mode"] == "full":
            system.train_parallel_ensemble(dataset,
            ...
            if t.get("feature_pruning_enabled", False):
                console.print("[yellow]Note:[/yellow] training.feature_pruning_enabled has no "
                              "effect under parallel partitioned training (not threaded through "
                              "worker processes) — ignoring.")
            system.train_parallel_partitioned(dataset,
                         epoch_count=t["epoch_count"],
                         judge_iterations=t["judge_iterations"],
                         loud=True,
                         judge_min_clusters=t.get("judge_min_clusters"),
...
def run_compare(settings: Settings, logger) -> None:
    ...
    console.print(f"[dim]Ignored columns: {d.get('ignored_columns') or 'none'}  |  "
    ...
    manager.run_all(
        ...
        connection_percentage = m.get("connection_percentage", 0.1),
        density        = m.get("density", 0.8),
    )
...
def _evaluate_system(system: SystemHandler, test_ds, target_col: str, m: dict, logger) -> None:
    ...
    with logger.make_progress() as prog:
        ...
        for row_dict in test_ds.to_dict(orient="records"):
