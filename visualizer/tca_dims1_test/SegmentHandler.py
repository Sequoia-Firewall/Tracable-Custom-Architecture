...
class SegmentHandler:
    ...
    def segmentInfer(self, pre_processed, loud=True, collect_paths: bool = False):
        """
        ...
        collect_paths : off by default — zero added cost unless explicitly
                       requested. When True, each report also includes
                       'paths': a list of node-position sequences, one per
                       collected signal, read straight off Signal.visited_nodes
                       (already populated every hop regardless of this flag —
                       forward_signal() appends to it unconditionally; this
                       flag only controls whether segmentInfer bothers to
                       read that list back out into the report). Built for
                       the visualization tool's signal-path tracing view —
                       see SystemHandler.runInfer(trace=...).
        ...
        def _run(bar=None):
            ...
            for rev in reviewers:
                ...
                if prediction is not None:
                    ...
                    report = {'id': self.segment_id, 'prediction': prediction,
                              'reviewer_position': rev.position, 'confidence': confidence}
                    if collect_paths:
                        report['paths'] = [
                            {
                                'positions': [list(n.position) for n in s.visited_nodes],
                                'final_prediction': s.prediction,
                                'variance': s.variance,
                            }
                            for s in rev_signals if s.is_active()
                        ]
