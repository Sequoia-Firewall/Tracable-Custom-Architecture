"""Generate a small real dimensions=1 model (2 segments) to verify the
viz tool actually works for 1D, not just 2D -- never tested this session."""
import sys, os, time
...
settings.override("model.dimensions", 1)
...
settings.override("training.judge_max_clusters", 2)

logger = RC.RichLogger(filename=f"smoke_dims1_{int(time.time())}.log", log_level=4, console_level=1)
