class Logger:
    def __init__(self, filename, log_level, console_level=1):
        ...
        self._logs_folder_ready = False  # avoids an os.path.exists() stat call on every log()

    def log(self, message, classification, Loud):
        # Check if folder exists, if not create it — only once per instance,
        # since this runs on every call (including hot-path debug messages).
