...
(20,000 rows), using settings.json's manually-tuned learning rate / grad
clip / delta clip / prediction range as-is (NOT the "auto" formulas --
see run_full_auto_parallel.py, whose auto-derived run scored R2~=0 on
this same dataset), with parallel partitioned training enabled.

Non-interactive equivalent of `python3 main.py --mode train` with only
training.parallel_enabled overridden, run directly against SystemHandler
so it doesn't block on main.py's confirmation prompts. Held-out test
metrics computed the same way as the auto run and the earlier
parallel-training validation scripts (disjoint test split the system
never trains on) so the two reports are directly comparable.
"""
...
def main():
    ...
    # Deliberately NOT touching prediction_range/learning_rate/grad_clip/
    # delta_clip -- keep this repo's manually-tuned settings.json values.
    # Only opt-in change: parallel training (TCA1.1.4).
    ...
    logger = RC.RichLogger(filename=f"full_manual_parallel_{int(time.time())}.log", log_level=4, console_level=2)
    ...
    print("\n=== Training (parallel partitioned, manually-tuned config) ===")
    t0 = time.time()
    ...
    report = {
        "config": {
            ...
            "lr_mode": t["learning_rate"]["mode"], "grad_clip_mode": t["grad_clip"]["mode"],
            "delta_clip_mode": t["delta_clip"]["mode"],
    ...
    with open("full_manual_parallel_report.json", "w") as f:
        json.dump(report, f, indent=2, default=str)

    print(f"\n=== DONE (total elapsed {report['total_elapsed_sec']:.0f}s) ===")
    print(f"Report written -> full_manual_parallel_report.json")
