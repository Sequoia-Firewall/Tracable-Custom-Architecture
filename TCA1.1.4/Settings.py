...
_DEFAULTS = {
    ...
    "training": {
        ...
        "parallel_enabled": False,  # experimental: train segments concurrently across OS
                                     # processes (SystemHandler.train_parallel_partitioned()
                                     # for training_mode='partitioned', train_parallel_ensemble()
                                     # for training_mode='full') instead of sequentially.
                                     # 1.1.4 validation (4 paired trials, varied data seeds):
                                     # ~3.2x average wall-clock speedup; the R2 difference vs
                                     # sequential was not statistically distinguishable from
                                     # noise at that sample size (paired t, n=4) — parallel even
                                     # won outright on 1 of 4 trials. Off by default because it
                                     # changes wall-clock/CPU behavior (spawns worker processes)
                                     # rather than because of a proven accuracy cost.
        "max_workers": None,  # passed to ProcessPoolExecutor when parallel_enabled=True.
                               # None -> os.cpu_count().
    },
    "logging": {
        "log_level": 4,
