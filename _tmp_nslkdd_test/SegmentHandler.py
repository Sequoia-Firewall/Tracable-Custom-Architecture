...
class SegmentHandler:
    ...
    def initializeSegment(self, loc = [], visualization_enabled = False): #loc used to determine if x or y is positive or negative
    ...
    LR_SCALE_REFERENCE_ROWS    = 2000   # avg per-node sample activations WEIGHT_LR/POSITION_LR/
                                         # SPLITTER_LR are tuned for (== train_rows in the
                                         # degenerate 1-node case; see _resolve_lr_schedule)
    ...
    def _auto_delta_clip(self, pred_min, pred_max):
        """Derive the forward-pass per-hop delta clip from this segment's actual
        node topology, rather than the target-span decade heuristic used by
        _auto_grad_clip.

        DELTA_CLIP acts directly on the propagated prediction (not a gradient
        buffered by a learning rate), so it must be tied to how many hops a
        signal actually takes to cross the segment, not just to target scale.
        initializeSegment() lays processing nodes out in
        ceil(num_nodes ** (1/dimensions)) shells between splitter and reviewer,
        and connect_nearest_nodes() only permits hops to strictly-farther-from-
        origin nodes, so a signal is structurally forced to cross roughly that
        many hops end to end. Dividing the target span by that hop count gives
        the average per-hop delta needed to reach the far end of the range
        within the segment's real hop budget.

        Falls back to the fixed DELTA_CLIP default when no usable range or
        node count is available.
        """
        if pred_min is None or pred_max is None:
    ...
    def _resolve_lr_schedule(self, lr_cfg, train_rows, epoch_count):
        ...
        'auto'         : max_lr_scale = clip(LR_SCALE_REFERENCE_ROWS / node_activations,
                          min_lr_scale, max_lr_scale), where node_activations =
                          train_rows * hops / num_nodes is the average number of
                          times a single processing node gets updated per epoch.
                          Signals take one stochastic path of ~hops nodes from
                          splitter to reviewer per sample (forward_signal's
                          random.choices routing — same hop estimate used by
                          _auto_delta_clip: ceil(num_nodes ** (1/dimensions))),
                          so a fixed row count spread over a bigger segment
                          graph means each node is updated less often, not
                          more, per epoch. Reducing to train_rows alone
                          (num_nodes<=1) reproduces the original row-only
                          formula exactly, so a bigger graph now gets a
                          proportionally bigger LR to compensate for its
                          diluted per-node update frequency, and a smaller one
                          gets a smaller LR, instead of every max_x sharing the
                          same trajectory whenever train_rows alone would clamp
                          to the floor/ceiling. min_lr_scale/max_lr_scale here
                          are just clamp bounds on that ratio. min_lr_scale for
                          the curve floor is then set to 0.05 * max_lr_scale,
                          and decay is solved so the curve reaches that floor
                          exactly on the last epoch.
        ...
        if train_rows:
            ...
            if num_nodes > 0:
                hops = max(1, math.ceil(num_nodes ** (1.0 / max(self.dimensions, 1))))
                node_activations = train_rows * hops / num_nodes
            else:
                node_activations = train_rows
            max_scale = min(max(self.LR_SCALE_REFERENCE_ROWS / node_activations, floor), ceiling)
        else:
            max_scale = ceiling
    ...
    def _epoch(self, sample_list, lr_w, lr_p, lr_s, desc, freeze_connections=False,
        ...
        reconnect_pct : fraction of this epoch's samples between mid-epoch
        ...
        # Reconnecting every processing node after every single sample (a full
        # nearest-neighbour pass per node) is the dominant cost of training —
        # positions only drift a small amount per sample, so routing doesn't
        # need to be refreshed that often. Throttle to roughly every
        # reconnect_pct of the epoch's samples instead; a final unconditional
        # reconnect after the loop (below) still guarantees topology matches
        # the true final positions regardless of where this lands.
        # reconnect_pct <= 0 reconnects every sample (throttling disabled).
        if sample_list and reconnect_pct > 0:
            reconnect_every = max(1, round(reconnect_pct * len(sample_list)))
        else:
            reconnect_every = 1
        ...
        # Post-epoch reconnect so topology reflects final positions (skipped
        # during plateau). Necessary every time now that in-loop reconnects
        # are throttled — the throttled schedule generally does not land on
        # the final sample, so this is the only guarantee that end-of-epoch
        # connectivity matches the true final positions.
    ...
    def train(self, dataset, epoch_count=3, preprocessor=None, lr_scale_cfg=None,
        ...
        delta_clip_cfg : optional dict {'mode', 'value'} (settings.training.delta_clip).
                       'manual' uses value directly; 'auto' derives the per-hop
                       forward-pass delta clip from this segment's actual node
                       topology (target span / hop count), so the fixed default
                       (tuned for a ~2-digit target range at this segment's default
                       geometry) doesn't cap predictions well below the target
                       range for larger target magnitudes. Requires pred_min/
                       pred_max to be set — otherwise falls back to each node's
                       built-in DELTA_CLIP default.
        visualization_enabled : whether to render a structure PNG after every
                       epoch (dimensions==2 only; see settings.output.visualization_enabled).
                       Off by default — purely a debugging aid, not needed for
                       training correctness, and the render + disk write add up
                       across epochs/segments.
        reconnect_pct : fraction of each epoch's samples between mid-epoch
                       topology reconnects (settings.training.reconnect_pct).
        ...
        # 80/20 train/test split. max(1, ...) alone degenerates when a
        # segment gets very few assigned rows (e.g. a tiny/imbalanced
        # JudgeNode cluster): len==1 gave split=1, leaving test_list empty
        # and crashing the eval_sample lookup below. min(..., len-1) keeps
        # at least one row on each side whenever there are >=2 samples at
        # all; the truly-degenerate 0-or-1-sample case is handled where
        # eval_sample is picked.
        split       = max(1, int(len(all_samples) * 0.8))
        if len(all_samples) >= 2:
            split = min(split, len(all_samples) - 1)
        ...
        # ── Delta clip (overrides default DELTA_CLIP) ───────────────────
        if delta_clip_cfg:
            if delta_clip_cfg.get('mode') == 'manual':
                delta_clip = delta_clip_cfg.get('value')
            else:
                delta_clip = self._auto_delta_clip(pred_min, pred_max)
            for node in sc['processing_nodes']:
                node.set_delta_clip(delta_clip)
            self.display(
                f"  Delta clip set to {delta_clip:.4g} "
                f"({delta_clip_cfg.get('mode')}).",
                classification=4
            )
        ...
        # Eval sample taken from test split so it was never seen during
        # training -- falls back to the last training row only in the
        # truly-degenerate case (<2 total samples for this segment, so the
        # split above couldn't carve out any held-out row at all). Only
        # affects this one illustrative per-epoch display sample; the real
        # metrics (_eval_epoch_metrics/_eval_test_predictions) already
        # handle an empty test_list correctly (return NaN), untouched here.
        if test_list:
            eval_sample = test_list[0]
        else:
            self.display(
                "No held-out test row for this segment (too few assigned samples) -- "
                "using a training row for the per-epoch display sample only.",
                classification=3
            )
            eval_sample = sample_list[-1]
        ...
        def _run_epochs(progress=None, epoch_task=None):
            ...
            for epoch in range(epoch_count):
                ...
                if visualization_enabled and self.dimensions == 2:
