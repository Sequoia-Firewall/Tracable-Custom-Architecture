...
class ComparisonManager:
    ...
    def _job_hash(self, dataset_path: str, target: str,
                  ...
                  connection_percentage: float | None = None,
    ...
    def run_all(
        ...
        delta_clip_cfg : optional {'mode', 'value'} — forwarded to SegmentHandler/
                         SystemHandler training exactly as in normal train mode
                         (see settings.training.delta_clip).
        reconnect_pct : fraction of each epoch's samples between mid-epoch
                         topology reconnects — forwarded to SegmentHandler/
                         SystemHandler training exactly as in normal train mode
                         (see settings.training.reconnect_pct). 0 disables
        ...
        ignored_columns : optional list of raw column names to drop before
                         ...
                         SegmentHandler/SystemHandler alike.
        shuffle       : whether to shuffle rows before the train/test split
                         (see settings.dataset.shuffle). Off by default so
                         existing callers/notebooks keep their current
                         positional-split behavior unless they opt in.
        shuffle_seed  : fixed seed for the shuffle so repeated runs (and
                         checkpoint resume, which hashes this value) reproduce
                         the same split.
        test_split    : fraction of rows held out for testing (see
                         settings.training.test_split).
        connection_percentage, density : SegmentHandler/SystemHandler topology
                         params (see settings.model.connection_percentage /
                         settings.model.density) — forwarded to every
                         SegmentHandler/SystemHandler comparison job.
        """
        ...
        preprocessor = PreProcesingNode(Logger=self.logger, logger_classification=4,
                                         removable_columns=ignored_columns or None)
        ...
        train_pct = round((1 - test_split) * 100)
        self._rule(f"Step 2 — Single Train / Test Split  ({train_pct} / {100 - train_pct})")
        records = processed_df.to_dict(orient='records')
        if shuffle:
            import random as _random
            records = list(records)
            _random.Random(shuffle_seed).shuffle(records)
            self._log(f"Shuffled {len(records)} rows (seed={shuffle_seed}) before splitting.")
        split   = max(1, int(len(records) * (1 - test_split)))
